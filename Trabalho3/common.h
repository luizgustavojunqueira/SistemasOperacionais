...
typedef struct {
  ...
  short type;    /* the file i-node type, DIRECTORY, FILE_TYPE (there's another
                    value FREE_INODE which never appears here */
...
/*	Note that this struct only allocates space for the size element.
        ...
        To use a message with a body of 50 bytes we must first allocate space
   for the message: char space[sizeof(int) + 50]; Then we declares a msg_t *
   variable that points to the allocated space: msg_t *m = (msg_t *) space; We
   can now access the size variable: m->size (at memory location &(space[0]) )
