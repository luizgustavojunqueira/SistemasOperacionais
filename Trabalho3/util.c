...
int peek_screen(int x, int y) { return screen[y * 80 + x] & 0xff; }
...
#ifndef FAKE
//	define a local prototype to int cpuspeed(), so we don't get any compiler
//warnings
int cpuspeed(void);
...
static uint64_t ms_to_cycles(uint32_t msecs) {
  ...
  result =
      cpu_spd * 1000; // cycles pr ms : (MHz * 10 ^ 6) * 10 ^ -3 = MHz * 10^3
  result *= msecs;
  return result;
}
